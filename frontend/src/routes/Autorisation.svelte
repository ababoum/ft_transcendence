<script lang="ts">
	...
	import { get_current_user_data } from "../stores";
	...
	onMount(async () => {
		let check:boolean = await get_current_user_data().then(
			resp => resp.is_logged_in
		);
		if (check)
			push("/");
	});
	...
	const submit = async () => {
		if (!is_login) {
			...
			await fetch(`${GAME_DOMAIN}/users/create`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ login, password, email, nickname }),
			}).catch((error) => {
				result_msg = "Can't register account: " + error;
			});
			if (result_msg == "Account was successfully registered")
				is_login = true;
		} else {
			let token;
			await fetch(`${GAME_DOMAIN}/auth/login`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
			...
			if (token != undefined) {
				...
				await push("/");
			} else result_msg = "Wrong login or password";
		}
	};
</script>
				...
				<div class="col-12 col-md-8 col-lg-6 col-xl-5">
					...
					>
						<div class="card-body p-5 text-center">
							<form on:submit|preventDefault={submit}>
								<div class="mb-md-5 mt-md-4 pb-5">
									...
									<p class="text-white-50 mb-5">
										Please enter your login{!is_login
											? ", email"
											: ""}
										and password!
									</p>
							...
							<div>
								<p class="mb-0">
									{is_login ? "Don't h" : "H"}ave an account?
									<a
										class="text-white-50 fw-bold"
										href={$LOGIN_URL}
										on:click={() => (is_login = !is_login)}
										>Sign {is_login ? "Up" : "In"}
									</a>
								</p>
							</div>
